...
def src(q, n):
    pld={"part": "snippet", "type": "video", "maxResults": n, "q": q, "key": ytapikey.getkey()}# "AIzaSyC89Pcgcts9ZRY4QCGWLq_3RGkjF-Hihdk"}
    ...
    if r.status_code==requests.codes.ok:
        ...
        for items in j["items"]:
            try:
                ...
                print("ERROR WHILE SEARCHING:", j)
    return resp

def getserverip():
	return open("SERVER_IP").read()
...
def dwd(art, sng, vid, fn):
    ...
    if r['error']:
        ...
        rq=requests.get("http://54.93.104.165/youtubemp3/ytmp3/PRJ/"+r["file"])
        f=open(fn, "wb")
        f.write(rq.content)
        f.close()
        print("Saved in \""+fn+"\"")
